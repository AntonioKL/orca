...
const appBundlePath = path.resolve(path.dirname(appPath), '..', '..')
...
const appEnv = {
  ...
  ORCA_PR18505_QA_TOKEN: launchToken,
...
function processMatchesHarness(pid, executablePath) {
  const result = spawnSync('/bin/ps', ['eww', '-p', String(pid), '-o', 'command='], {
    encoding: 'utf8',
    timeout: 5_000
  })
  return result.status === 0 &&
    result.stdout.includes(executablePath) &&
    result.stdout.includes(`ORCA_PR18505_QA_TOKEN=${launchToken}`)
}

function recordOwnedPid(pid, executablePath) {
  if (!processMatchesHarness(pid, executablePath)) {
    throw new Error(`refusing to record unverified PID ${pid}`)
  }
  ownedPids.set(pid, executablePath)
}

function stopOwnedPids() {
  for (const [pid, executablePath] of ownedPids) {
    if (!isPidAlive(pid)) continue
    if (!processMatchesHarness(pid, executablePath)) continue
...
function invokeBounded(args, lane, trafficLog, summary) {
  return new Promise((resolve) => {
    ...
    if (child.pid) recordOwnedPid(child.pid, cliPath)
    const chunks = []
...
const launchEnvironmentArgs = [
  ...
  'HOME',
  'USERPROFILE',
  'ORCA_PR18505_QA_TOKEN',
  'ELECTRON_ENABLE_LOGGING'
].flatMap((key) => ['--env', `${key}=${appEnv[key]}`])
...
if (openResult.status !== 0) {
  throw new Error(`Launch Services failed: ${openResult.stdout} ${openResult.stderr}`)
}
const sourceAppPid = await waitFor(() => {
  const result = spawnSync('/bin/ps', ['-axo', 'pid='], { encoding: 'utf8', timeout: 5_000 })
  if (result.status !== 0) return false
  const matches = result.stdout
    .split('\n')
    .map((value) => Number(value.trim()))
    .filter((pid) => Number.isInteger(pid) && pid > 0 && processMatchesHarness(pid, appPath))
  if (matches.length > 1) throw new Error(`multiple exact hosted app PIDs found: ${matches.join(',')}`)
  return matches[0] ?? false
}, 30_000, 'Launch Services app PID', 250)
recordOwnedPid(sourceAppPid, appPath)
writeFileSync(path.join(outputDir, 'source-launcher-pid.txt'), `${sourceAppPid}\n`)
markStage('source-launched-via-launch-services', { pid: sourceAppPid })

const [{ browser, page }, sourceRuntime] = await Promise.all([attachCdp(), waitForRuntime(null, 'source runtime')])
if (sourceRuntime.pid !== sourceAppPid) {
  throw new Error(`runtime PID ${sourceRuntime.pid} != Launch Services PID ${sourceAppPid}`)
}
markStage('source-runtime-and-renderer-ready', { pid: sourceRuntime.pid, runtimeId: sourceRuntime.runtimeId })
recordOwnedPid(sourceRuntime.pid, appPath)
...
recordOwnedPid(replacementRuntime.pid, appPath)
