...
export function describeMutationRequestState(params: {
  ...
  if (state === 'pending') {
    return `Request ${requestId} was accepted${method ? ` by ${method}` : ''} but its outcome was never recorded, so Orca restarted mid-mutation. Replay the original command with --retry-request ${requestId}; do not issue a fresh mutation.`
  }
