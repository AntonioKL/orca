...
export async function installRemoteGrokHook(
  ...
  try {
    ...
    // Why the same guard as local: a remote host runs standalone Grok sessions too, and without it
    // every tool event there spawns a shell for the managed script before discovering Orca is not
    // attached. ORCA_PANE_KEY is only present on panes Orca launched.
