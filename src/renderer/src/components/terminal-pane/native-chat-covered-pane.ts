...
const NATIVE_CHAT_ROOT_SELECTOR = '[data-native-chat-root]'
 ...
 */
export const UNCOVERED_TERMINAL_LEAF_SELECTOR = `[data-leaf-id]:not(:has(${NATIVE_CHAT_ROOT_SELECTOR}))`

export function paneIsCoveredByNativeChat(
  pane: { container: Pick<Element, 'querySelector'> } | null | undefined
): boolean {
  return pane?.container.querySelector(NATIVE_CHAT_ROOT_SELECTOR) != null
}
