...
describe('guardRunningTerminalClose child-process evidence', () => {
  // Why these exist: the guard used to read `hasChildProcesses` alone, and a host that could not
  // look spells that the same way as a host that looked and found nothing. The Windows SSH relay
  // spelled it `false` unconditionally, so a pane running a build closed with no prompt.
  beforeEach(() => {
    vi.clearAllMocks()
    setState()
  })
  ...
  it('asks the host to pay for a scan, because a close decides once and destructively', async () => {
    inspectRuntimeTerminalProcessMock.mockResolvedValue({
  ...
  it('prompts when the host could not observe the pane, rather than reading it as idle', async () => {
    inspectRuntimeTerminalProcessMock.mockResolvedValue({
      foregroundProcess: 'cmd.exe',
      hasChildProcesses: false,
  ...
  it('closes without a prompt only when the host observed an empty pane', async () => {
    inspectRuntimeTerminalProcessMock.mockResolvedValue({
      foregroundProcess: 'cmd.exe',
      hasChildProcesses: false,
      childProcessEvidence: 'no-children'
    ...
    expect(onClose).toHaveBeenCalledTimes(1)
    expect(visibleRequest()).toBeFalsy()
  })
