...
## 2. Remove the shared fate between auth and relay (this quarter)

| # | Item | Why | How | Size |
|---|---|---|---|---|
| 2.1 | **Private IP for Cloud SQL, `--private-ip` on the cell proxies** | Cells reach the database's public IP through Cloud NAT. Dynamic port allocation (landed) raised the ceiling from 64 to 4096 ports per VM, but the NAT is still in the path and its logs are still the only place port exhaustion shows up (Finding 11). | Add a private IP to `orca-cloud-auth-db` (foundation root, orca-cloud), peer the relay VPC, switch the proxy flag in the cell template, roll. | 1–2 days |
| 2.2 | **Split the relay database from the auth database** | One Cloud SQL instance serves `orca_auth`, `orca_relay`, `orca_push`, `orca_skills`. The auth table's growth stalled the relay for a day (Findings 10, 13). A login incident should not be able to take down the data plane, and a relay lock storm should not slow sign-in. | New instance for `orca_relay`; migrate with a short relay drain (the same-cap machinery already drains cells). Relay state is small (assignments, cells, leases) so the cutover is minutes. | 1–2 weeks incl. rehearsal on staging |
