...
- **Windows EDR signal**: don't add `-ExecutionPolicy Bypass`, `-EncodedCommand`, `cmd.exe /c` with escaped free text, per-operation interpreter spawning, or runtime `Add-Type` compilation without reading [`docs/reference/windows-edr-posture.md`](./docs/reference/windows-edr-posture.md) first.
...
Behavioural EDR (Microsoft Defender for Endpoint and peers) scores Orca's Windows process tree, and several of our legitimate mechanisms are attack-technique-shaped: encoded policy-bypassed PowerShell, `cmd.exe /c` carrying escaped free text, per-operation interpreter spawning, runtime `Add-Type` compilation, reading every process's memory on a cadence, and copying our own image under a different name. Before adding any of those on Windows, follow [`docs/reference/windows-edr-posture.md`](./docs/reference/windows-edr-posture.md) — it maps each shape to the code that produces it, the MITRE technique a tenant will map it to, and what to reach for instead. Signing does not clear these; only behaviour and prevalence do.
