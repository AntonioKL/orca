...
- The response was lost and named no Dispatch: run `orca orchestration request-show --request <request_id> --json` first. It is read-only. `completed` means the mutation already took effect and `pending` means Orca restarted mid-mutation; in both cases replay the original command with `--retry-request <request_id>`, which returns the recorded outcome instead of starting a second operation. `absent` means this runtime holds no receipt under your caller identity and is not proof that nothing happened.
