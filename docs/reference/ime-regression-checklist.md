...
one machine. Rows marked **(pending)** describe a contract whose implementation
has not merged yet; they are the acceptance criteria that change must meet, not
...
not exercise the operating system's input method.

**No CI job on the PR gate runs a real input method.** `pr.yml` is `ubuntu-latest`
(plus one `windows-2022` package job) and its unit tests run under happy-dom;
`e2e.yml` drives composition through CDP `Input.imeSetComposition`, which is a
synthetic composition. `terminal-ime-e2e.yml` is the only workflow with a real
engine (Xvfb + nested GNOME Shell + ibus-hangul + xdotool), and it is nightly
`schedule` plus `workflow_dispatch` — it gates nothing. There is no macOS runner
on the PR gate at all, so macOS-only IME code executes zero times.

So a green PR means the synthetic contracts hold. It does not mean any real IME
ran. The evidence below has to be produced deliberately:
