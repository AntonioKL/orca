...
| [#16949](https://github.com/stablyai/orca/issues/16949) terminal preedit has no visible cursor               | The opaque composition overlay covers the terminal renderer's cursor without painting its own.                                      | The existing xterm `CompositionHelper` layer owns a visible caret immediately after the preedit and before any visible row remainder, without a second renderer-specific blink loop.                         | Start, update, right-edge placement, cleanup, and update-without-start are covered; preview and normal terminals inherit the live cursor theme.                           |
...
| [#17104](https://github.com/stablyai/orca/issues/17104) Korean preedit repeats the Codex placeholder         | The composition overlay copies buffer text without carrying its dim metadata.                                                       | A wholly dim row remainder keeps its overlay width but is hidden; any content-bearing non-dim cell keeps the remainder visible.                                                                              | Full placeholder mask, mixed dim/non-dim tail, style-only repaint, and the ordinary mid-line row-tail regression are covered.                                             |
